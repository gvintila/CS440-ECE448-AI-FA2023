...
class Maze:
    ...
    def filter_valid_waypoints(self):
        ...
        for shape_idx in valid_waypoints:
            for w in self.get_waypoints():
                test_alien = self.create_new_alien(w[0], w[1], shape_idx)
                if not does_alien_touch_wall(test_alien, self.walls):
                    valid_waypoints[shape_idx].append(w)
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        candidate_neighbors = []
        for end_waypoint in self.__valid_waypoints[cur_shape]:
            end_shape = cur_shape
            end = (end_waypoint[0], end_waypoint[1], end_shape)
            if cur_waypoint != end_waypoint and self.is_valid_move(start, end):
                candidate_neighbors.append( (end_waypoint, euclidean_distance(end_waypoint, cur_waypoint)) )
        candidate_neighbors.sort(key = lambda x: x[1]) # Sort by distance, closest to greatest
        nearest_neighbors = [candidate[0] for candidate in candidate_neighbors]
        if self.k > len(nearest_neighbors): # If k is bigger than array, return whole array
            return nearest_neighbors
        else:
            return nearest_neighbors[0:self.k]
    ...
    def is_valid_move(self, start, end):
        ...
        """
        if hash(tuple([start, end])) in self.move_cache:
            return self.move_cache[hash(tuple([start, end]))]
        
        if start[2] > 2 or start[2] < 0 or end[2] > 2 or end[2] < 0: # Check for shape range
            self.move_cache[hash(tuple([start, end]))] = False
            return False
        ...
        # Change shape
            ...
            # Alien cannot move while changing shapes
            if start[0] != end[0] or start[1] != end[1]:
                print("Error, shape and movement change detected")
                self.move_cache[hash(tuple([start, end]))] = False
                return False
            
            start_alien.set_alien_shape( start_alien.get_shapes()[end[2]] )
            if does_alien_touch_wall( start_alien, self.walls ):
                self.move_cache[hash(tuple([start, end]))] = False
                return False
        
        self.move_cache[hash(tuple([start, end]))] = True
        return True
