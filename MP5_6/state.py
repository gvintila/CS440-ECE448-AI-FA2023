...
class MazeState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        nbr_states = []
        for nbr_neighbor in self.maze_neighbors(self.state[0], self.state[1], self.state[2]):
            dist_start_end = euclidean_distance(self.state, nbr_neighbor)
            if self.state[2] != nbr_neighbor[2]:
                nbr_states.append(MazeState(nbr_neighbor, self.goal, self.dist_from_start + 10, self.maze, self.use_heuristic))
            else:
                nbr_states.append(MazeState(nbr_neighbor, self.goal, self.dist_from_start + dist_start_end, self.maze, self.use_heuristic))
        return nbr_states

    # TODO VI
    def is_goal(self):
        # If state and goal coords match, return true
        if self.state[0:2] in self.goal:
            return True
        else:
            return False
    ...
    def compute_heuristic(self):
        c = -1
        for g in self.goal:
            if c == -1 or euclidean(self.state[0:2], g) < c:
                c = euclidean(self.state[0:2], g)
        return c
